...
def render_team_overview():
    ...
    st.markdown("### 📈 Säsongsöversikt (Lagets snitt per match)")
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Mål xG per match", "5.0+", "Målsättning")
    col2.metric("Aktuellt Snitt xG", "4.2", "+0.4 vs föregående")
    col3.metric("Snitt Gjorda Mål", f"{avg_gf:.1f}", "Faktiska mål framåt")
    col4.metric("Snitt Insläppta Mål", f"{avg_ga:.1f}", "Faktiska mål bakåt")

    col5, col6, col7, col8 = st.columns(4)
    col5.metric("Motståndarens Snitt xG", f"{avg_opp_xg:.2f}", "Motståndar-xG per match")
    col6.metric("Defensivt xGA-snitt", "1.8", "Stabil defensiv")
    col7.metric("Snitt Slot Completions", "11.5", "Per match")
    col8.metric("Antal spelade matcher", f"{len(df_matches) if not df_stats.empty else 0}", "Registrerade")
...
def main_app():
    ...
    # 1c. Mina Utvecklingsmål (Spelare & Tränare)
        ...
        # Formulär för att lägga till mål
    ...
    # 5. Min Statistik & Trender (Med grafer och export)
        ...
        if df_player.empty:
            ...
            st.markdown("---")
            # Exportknapp för spelarens egna data
