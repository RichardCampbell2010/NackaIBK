...
# Detaljerad match- och spelarhistorik per match (för trender och positionsbyten per match)
# Format: [{"MatchID": "Gamla Stan (2026-03-10)", "Spelare": "Moa Lindström", "Position": "Center", "Tid med boll": 45.0, ...}]
...
def render_glossary():
    ...
    with st.expander("📌 1. xG & Zoner"):
        st.markdown("Mäter sannolikheten för mål baserat på avslutsposition (Slottet vs Fickor).")
    with st.expander("📌 2. Slot Completions (Instick)"):
        st.markdown("Lyckade passningar direkt in i slottet som höjer xG drastiskt.")
...
def main_app():
    ...
    # 3. Hantera Truppen (Med grundposition)
    elif menu == "👥 Hantera Truppen":
        st.title("👥 Hantera Truppen")
        st.markdown("Lägg till spelare och ange deras grundposition (Back, Forward eller Center).")
            ...
            with st.form("add_player_form"):
                ...
                if add_sub and new_p_name:
                    if new_p_name not in st.session_state.player_data["Spelare"].values:
                        ...
                        st.session_state.player_data.to_json(PLAYERS_FILE, orient="split")
                        st.success(f"Lade till {new_p_name} ({new_p_pos})!")
                        st.rerun()
                    else:
    ...
    # 4. Registrera Matchstatistik (Där position kan ändras per match!)
    elif menu == "📈 Registrera Matchstatistik":
        st.title("📈 Registrera Matchstatistik & Spelarinsatser")
        st.markdown("Här matar du in statistik per spelare för en specifik match. Om en spelare byter position för dagen (t.ex. en back som spelar center), väljer du den positionen här för just den matchen!")
        ...
        with st.form("match_stats_form"):
            ...
            # Skapa inmatningsfält för varje spelare i truppen
            match_records = []
            for idx, row in st.session_state.player_data.iterrows():
                p_name = row["Spelare"]
                default_pos = row["Position"]
                
                st.markdown(f"**{p_name}**")
                mp_col1, mp_col2, mp_col3, mp_col4, mp_col5 = st.columns(5)
                
                # Tillåt att ändra position just denna match!
                p_pos_today = mp_col1.selectbox(f"Position i matchen ({p_name})", ["Back", "Forward", "Center"], index=["Back", "Forward", "Center"].index(default_pos), key=f"pos_{p_name}_{match_id}")
            ...
            if submit_match:
                ...
                # Ta bort eventuell tidigare data för samma matchID om man uppdaterar
    ...
    # 5. Min Statistik & Trender (Spelarvy / Tränarvy för enskild spelare)
    elif menu == "📊 Min Statistik & Trender":
        current_name = st.session_state.name if st.session_state.role == "player" else st.session_state.get("selected_admin_player", st.session_state.player_data["Spelare"].iloc[0] if not st.session_state.player_data.empty else "")
        ...
        if df_player.empty:
            ...
            # Beräkna trender (Senaste 3, Senaste 5, Säsongsstart/Total)
            ...
            st.markdown("Här jämförs spelarens snitt i de matcher hon spelat på en viss position mot *alla andra spelare* som spelade på samma position i samma matcher.")
            
            # Hämta spelarens senaste position eller visa per match
            ...
            # Jämför med lagets snitt för den positionen
    ...
    # 6. Matchhistorik & Trender (Övergripande)
