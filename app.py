...
# Filnamn för permanent lagring av truppen
...
# Ladda spelardata från fil om den finns, annars skapa standard och spara filen
if "player_data" not in st.session_state:
    if os.path.exists(PLAYERS_FILE):
        try:
            st.session_state.player_data = pd.read_json(PLAYERS_FILE, orient="split")
        except Exception:
            # Fallback om filen skulle vara korrupt
...
def main_app():
    ...
    # 3. Hantera Truppen (Med Spara-knapp!)
    elif menu == "👥 Hantera Truppen":
        st.title("👥 Hantera Truppen")
        st.markdown("Här lägger du till nya spelare i truppen eller tar bort spelare. Tryck på **Spara ändringar** när du är klar så ligger det kvar permanent.")
            ...
            if st.button("Lägg till i listan"):
                if new_p_name:
                    if new_p_name not in st.session_state.player_data["Spelare"].values:
                        ...
                        st.success(f"Lade till {new_p_name} i listan! (Glöm inte att trycka Spara nedan).")
                    else:
        ...
        with col_t2:
            ...
            if not st.session_state.player_data.empty:
                ...
                if st.button("Ta bort från listan"):
                    ...
                    st.success(f"Tog bort {rem_player} från listan! (Glöm inte att trycka Spara nedan).")
            else:
                st.info("Inga spelare kvar i truppen.")

        st.markdown("---")
        # Den efterlängtade SPARA-knappen!
        if st.button("💾 Spara ändringar permanent till truppen", type="primary"):
            st.session_state.player_data.to_json(PLAYERS_FILE, orient="split")
            st.success("Ändringarna har sparats! Nu ligger de kvar även om du laddar om sidan eller startar om appen.")
