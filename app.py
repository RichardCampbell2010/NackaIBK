...
def render_glossary():
    ...
    with st.expander("📌 1. xG (Expected Goals) & Zoner"):
        st.markdown("""
        * **xG (Expected Goals):** Mäter sannolikheten för att ett skott resulterar i mål baserat på historisk data och avslutsposition.
        * **Slottet:** Det gyllene målområdet framför mål som ger absolut högst xG-värde och målchans.
        * **Mål-målsättning xG:** Vårt uppsatta offensiva mål per match (satt till 5.0+).
        """)
        
    with st.expander("📌 2. Slot Completions (Instick)"):
        st.markdown("""
        * **Slot Completions:** Lyckade passningar eller genombrott direkt in i slottet. Detta är en av våra absolut viktigaste offensiva KPI:er eftersom instick i slottet radikalt ökar sannolikheten för mål.
        """)
        
    with st.expander("📌 3. Framåtdrivande aktioner"):
        st.markdown("""
        * **Framåtdrivande aktioner:** Aktier där spelaren driver bollen förbi motståndare eller slår ett uppspel som bryter motståndarens lagdelar och sätter fart på anfallet.
        """)
...
# --- SPELARENS INDIVIDUELLA STATISTIK (DELAD VY) ---
