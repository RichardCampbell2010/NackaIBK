...
# Ladda eller skapa spelardata (och säkra upp att kolumnen Position finns)
if "player_data" not in st.session_state:
    if os.path.exists(PLAYERS_FILE):
        try:
            st.session_state.player_data = pd.read_json(PLAYERS_FILE, orient="split")
            # Om gamla sparade filer saknar "Position"-kolumnen, lägg till den automatiskt
...
def main_app():
    ...
    # 4. Registrera Matchstatistik
        ...
        st.markdown("Här matar du in statistik per spelare för en specifik match. Om en spelare byter position för dagen, väljer du den här!")
        ...
        with st.form("match_stats_form"):
            ...
            for idx, row in st.session_state.player_data.iterrows():
                ...
                p_pos_today = mp_col1.selectbox(f"Position i matchen ({p_name})", ["Back", "Forward", "Center"], index=pos_index, key=f"pos_{p_name}_{match_id}")
