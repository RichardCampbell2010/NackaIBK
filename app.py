...
if "match_player_stats" not in st.session_state:
    if os.path.exists(MATCHES_FILE):
        ...
        st.session_state.match_player_stats = pd.DataFrame([
            {
                "MatchID": "Gamla Stan - 2026-03-10", "Datum": "2026-03-10", "Motståndare": "Gamla Stan",
                "Spelare": "Sonja Fredriksson", "Position": "Forward",
                "Tid med boll (s)": 45.0, "Framåtdrivande aktioner": 12, "Skott i Slottet": 4,
                "Slot Completions": 3, "Slot-effektivitet (%)": 33.3, "Vunna närkamper": 7,
                "Förlorade närkamper": 2, "Bolltapp (Egen zon)": 1, "Bolltapp (Off. zon)": 2,
                "Gjorda mål": 5, "Insläppta mål": 2, "Motståndare xG": 1.6, "Videolänk": ""
            },
            {
                "MatchID": "Gamla Stan - 2026-03-10", "Datum": "2026-03-10", "Motståndare": "Gamla Stan",
                "Spelare": "Elin Andersson", "Position": "Back",
                "Tid med boll (s)": 62.5, "Framåtdrivande aktioner": 18, "Skott i Slottet": 6,
                "Slot Completions": 5, "Slot-effektivitet (%)": 40.0, "Vunna närkamper": 10,
                "Förlorade närkamper": 3, "Bolltapp (Egen zon)": 0, "Bolltapp (Off. zon)": 1,
                "Gjorda mål": 5, "Insläppta mål": 2, "Motståndare xG": 1.6, "Videolänk": ""
            }
        ])
...
def render_glossary():
    ...
    with st.expander("📌 1. xG (Expected Goals) & Zoner"):
        st.markdown("Mäter sannolikheten för mål baserat på avslutsposition. Slottet ger högst xG-värde.")
    with st.expander("📌 2. Slot Completions (Instick)"):
        st.markdown("Lyckade passningar direkt in i slottet som radikalt ökar målchansen.")
    with st.expander("📌 3. Framåtdrivande aktioner"):
        st.markdown("Driv med boll eller uppspelsgenombrott som bryter motståndarens lagdelar.")

# --- GEMENSAM LAGÖVERSIKT (MED SID-VID-SID XG & FÄRGKODNING) ---
def render_team_overview():
    ...
    if not df_stats.empty:
        ...
        avg_gf, avg_ga, avg_opp_xg = 0.0, 0.0, 0.0

    st.markdown("### ⚡ Offensiv vs Defensiv xG (Jämförelse per match)")
    
    # Jämförelse Sida vid Sida: Vårt xG / Mål vs Motståndarens xG
    ...
    with col_our:
        ...
        oc2.metric("Aktuellt Snitt xG", "4.2", "+0.4 vs föregående", delta_color="normal")
        
        om1, om2 = st.columns(2)
        om1.metric("Snitt Gjorda Mål", f"{avg_gf:.1f}", "Faktiskt framåt", delta_color="normal")
        om2.metric("Snitt Slot Completions", "11.5", "Per match")

    with col_opp:
        st.markdown("#### 🛡️ Motståndare (Defensivt tryck)")
        mc1, mc2 = st.columns(2)
        # Lågt motståndar-xG är bra för oss -> grön delta-effekt
        mc1.metric("Motståndarens Snitt xG", f"{avg_opp_xg:.2f}", "-0.3 vs mål (Stabil)", delta_color="inverse")
        mc2.metric("Snitt Insläppta Mål", f"{avg_ga:.1f}", "Bakåt", delta_color="inverse")
        
        mm1, mm2 = st.columns(2)
        mm1.metric("Defensivt xGA-snitt", "1.8", "Godkänd nivå")
        mm2.metric("Antal spelade matcher", f"{len(df_matches) if not df_stats.empty else 0}", "Registrerade")
...
def main_app():
    ...
    # 1. Match & Lagtaktik (Tränare)
    if menu == "📊 Match & Lagtaktik":
        render_team_overview()

    # 1b. Lagets Översikt & Statistik (Spelare)
    elif menu == "🛡️ Lagets Översikt & Statistik":
        render_team_overview()

    # 1c. Mina Utvecklingsmål (Spelare & Tränare)
    ...
    # 5. Min Statistik & Trender
    elif menu == "📊 Min Statistik & Trender":
        current_name = st.session_state.name if st.session_state.role == "player" else st.session_state.player_data["Spelare"].iloc[0] if not st.session_state.player_data.empty else ""
        
        st.title(f"📊 Statistik & Trender: {current_name}")
        
        df_player = st.session_state.match_player_stats[st.session_state.match_player_stats["Spelare"] == current_name]
        
        if df_player.empty:
            st.warning("Inga matcher sparade för denna spelare ännu.")
        else:
            ...
            st.download_button(
                label="📥 Ladda ner din statistik som CSV",
                data=csv_data,
                file_name=f"statistik_{current_name.replace(' ', '_')}.csv",
                mime="text/csv"
            )

    # 6. Matchhistorik & Trender (Tränare) - MED SPARAD RADERING
        ...
        if st.session_state.match_player_stats.empty:
            ...
            # Hämta unika MatchID (t.ex. "Älvsjö AIK - 2026-10-04")
            ...
            if col_del1.button("🗑️ Radera matchen permanent"):
                # Ta bort från session state
                st.session_state.match_player_stats = st.session_state.match_player_stats[
                    st.session_state.match_player_stats["MatchID"] != match_id_to_del if 'match_id_to_del' in locals() else st.session_state.match_player_stats["MatchID"] != match_to_delete
                ]
                # Spara direkt till fil så den inte kommer tillbaka vid omstart!
                st.session_state.match_player_stats.to_json(MATCHES_FILE, orient="split")
